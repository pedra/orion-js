...
// GetElementById
function _(e) {
	return document.querySelector(e) || false
}

// GetSelectorAll
function _a(e) {
	return document.querySelectorAll(e) || false
}
...
function glass(text, vitrine) {
	_('#w5_media_glassText').innerHTML = 'string' == typeof text ? text : ''
	_('#w5_media_glassText').style.display = 'string' == typeof text ? 'block' : 'none'
	_('#w5_media_glass').style.display = text === false ? 'none' : 'flex'
	_('#w5_media_glass').classList[vitrine === true ? 'add' : 'remove']('vitrine')
}
...
function textToHtml(t) {
	return t
		...
		.replace(/([^\w]|\s|\*|\-)(\_([^/s]|.*?)\_)([^\w]|\s|\*|\-)/g, '$1<i>$3</i>$4') // _italic_
...
function progress(on) {
	var on = on || false
	_('#w5_media_progress').style.display = on ? 'block' : 'none'
}
